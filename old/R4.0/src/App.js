...
//        setPointsData(pointsWithManual);
        ...
        for (let i = 0; i < pointsData.length; i++) {
            ...
            // ★ 勾配によるペース補正（上り・下り両方を考慮） ★
            const gradient = currentPoint.section_gradient || 0;
            let gradientBasedPenalty = 0;

            if (gradient > 2) { // 上り坂
                if (gradient <= 5) {
                    gradientBasedPenalty = 150;
                } else if (gradient <= 10) {
                    gradientBasedPenalty = 220;
                } else if (gradient <= 15) {
                    gradientBasedPenalty = 300;
                } else {
                    gradientBasedPenalty = 400;
                }
            } else if (gradient < -5) { // 下り坂
                if (gradient <= -15) {
                    gradientBasedPenalty = -15; // 急な下りはペースアップ
                } else if (gradient <= -10) {
                    gradientBasedPenalty = -10;
                } else { // (gradient > -10 && gradient <= -5)
                    gradientBasedPenalty = -5;
                }
            } else { // 緩やかな上り(-2～2%)、緩やかな下り(-5～-2%)、平坦
                gradientBasedPenalty = 0;
            }
            currentPaceInSecondsPerKm *= (1 + (gradientBasedPenalty / 100));
