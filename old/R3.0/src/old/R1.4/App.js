...
function App() {
  ...
  }, [setGpxFileLoaded, setPointsData, setSimulationResult, setTableData, setFirstKmArrivalTime]);
  ...
  console.log('pointsData on update:', pointsData.slice(0, 5)); // 最初の5件をログ出力
...
console.log('calculatePaceWithITRA called with:', {
  basePaceInSecondsPerKm,
  itraIndexValue,
  distanceRatio,
});
  
    // distanceRatio: レース全体の距離に対する現在の地点の距離の割合 (0〜1)
    let modifiedPace = basePaceInSecondsPerKm;

  if (!isNaN(itraIndexValue)) {
    const itraDiff = itraIndexValue - 400;
    console.log('itraDiff:', itraDiff);

    if (itraDiff < 0) {
      console.log('ITRA index is low (< 400)');
      if (distanceRatio < 0.3) {
        const earlyRacePacePenaltyFactor = (0.3 - distanceRatio) / 0.3;
        const earlyRacePacePenalty = (Math.abs(itraDiff) / 400) * 0.1 * earlyRacePacePenaltyFactor;
        modifiedPace *= (1 + earlyRacePacePenalty);
        console.log('earlyRacePacePenaltyFactor:', earlyRacePacePenaltyFactor, 'earlyRacePacePenalty:', earlyRacePacePenalty, 'modifiedPace (low ITRA, early):', modifiedPace);
      } else {
        console.log('Not in early race (distanceRatio >= 0.3)');
      }
    } else if (itraDiff > 0) {
      console.log('ITRA index is high (> 400)');
      if (distanceRatio > 0.7) {
        const lateRacePaceFactor = 1 + (itraDiff / 600) * 0.05 * ((distanceRatio - 0.7) / 0.3); // ★ 1 に加算する ★
        modifiedPace /= lateRacePaceFactor;
        console.log('lateRacePaceFactor:', lateRacePaceFactor, 'modifiedPace (high ITRA, late):', modifiedPace);
      } else {
        console.log('Not in late race (distanceRatio <= 0.7)');
      }
    }
  }

 console.log('calculatePaceWithITRA returns:', modifiedPace);
...
console.log('handleSimulate called, itraIndexValue:', itraIndexValue); // ★ ログ出力 ★
...
console.log('Total distance before loop:', totalDistance); // ★ 追加 ★
console.log('pointsData.length before loop:', pointsData.length); // ★ 追加 ★
...
console.log(`Point ${i}, distanceRatio: ${distanceRatio}, pace (after gradient): ${currentPaceInSecondsPerKm}, gradient: ${gradient}`); // ★ gradient の値も出力 ★
...
console.log('Max distanceRatio in this simulation:', maxDistanceRatio);
