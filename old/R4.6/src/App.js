...
function App() {
    ...
    const [draftManualPoints, setDraftManualPoints] = useState([{ id: uuidv4(), distance: '', type: '', name: '', restTime: '', gateTime: '' }]);
...
console.log(`Index: ${i}, 距離: ${currentPoint.cumulative_distance.toFixed(3)}, 標高: ${currentPoint.elevation}, 勾配: ${gradient.toFixed(3)}`);
                ...
                <div>
                    ...
                    {draftManualPoints.map((point, index) => (
                        <div key={point.id} style={{ display: 'flex', alignItems: 'center', marginBottom: '10px', justifyContent: 'center' }}>
                            ...
                            {/* ★ 関門時間入力欄 ★ */}
                            <div style={{ display: 'flex', alignItems: 'center', marginRight: '10px' }}>
                                <label style={{ marginRight: '5px' }}>関門時間 (HH:MM):</label>
                                <input
