...
console.log("handleFileSelected 開始");
...
console.log("saveManualPoints 開始");
...
console.log("handleGPSDataImport 開始");
...
console.log("handleApplyManualPoints を呼び出します");
                }
...
console.log("handleStartTimeChange 開始");
        setStartTime(event.target.value);
    }, []);

    // ★ 追加: FINISH時間変更ハンドラ ★
    const handleFinishTimeChange = useCallback((e) => {
console.log("handleFinishTimeChange 開始");
...
console.log("handleItraIndexChange 開始");
...
console.log("handleFlatPaceInputChange 開始");
        setFlatPaceInput(event.target.value);
    }, []);

    const calculatePaceFromFinishTime = useCallback(() => {
console.log("calculatePaceFromFinishTime 開始");
...
console.log("handleDraftManualPointChange 開始");
...
console.log("applyManualPointsToPointsData 開始");
...
console.log("handleApplyManualPoints() が実行された");
     if (!gpxFileName) {
      alert("GPXファイルが読み込まれていません。");
      return;
     }
console.log("gpxFileName =", gpxFileName);
console.log("適用する手動ポイント:", draftManualPoints);
...
//debug
useEffect(() => {
    console.log("manualPointsApplied の変更を検出: 現在 =", manualPointsApplied);
}, [manualPointsApplied]);
//
...
console.log("シミュレーション開始: manualPointsApplied =", manualPointsApplied);
if (!manualPointsApplied) {
    console.warn("⚠️ 手動ポイントが適用されていない可能性があります！");
}
...
//console.log(`Index: ${i}, 距離: ${currentPoint.cumulative_distance.toFixed(3)}, (${(cumulativeDistance - previousCumulativeDistance).toFixed(3)}), 標高: ${Math.round(currentPoint.elevation)}, 勾配: ${gradient.toFixed(3)}`);
            ...
            if (currentPoint.区分 !== '') {
console.log("currentPoint.区分", currentPoint.区分);
