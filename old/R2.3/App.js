  ...
  const saveManualPoints = useCallback((gpxFileName, currentManualPoints) => {
    ...
    if (storageKey) {
      try {
        ...
        console.log(`手動地点情報を保存しました: ${storageKey}`); // ★ key を storageKey に修正 ★
...
  }, []);
  ...
  const handleApplyManualPoints = useCallback(() => {
    if (!gpxFileName) {
      console.log('gpxFileName is still undefined, waiting...');
      setTimeout(() => {
        if (gpxFileName) {
          ...
          alert("GPXファイルが読み込まれていません (timeout).");
        }
      }, 500); // 500ミリ秒待つ
      return;
    }
...
        // ★ 手動で追加された地点の場合、休憩時間を加算 ★
        if (currentPoint.manual && currentPoint.restTime) {
          const restTimeInMinutes = parseInt(currentPoint.restTime, 10);
          if (!isNaN(restTimeInMinutes) && restTimeInMinutes > 0) {
            currentTime.setTime(currentTime.getTime() + restTimeInMinutes * 60 * 1000);
          }
        }
      }
...
        restTime: currentPoint.restTime, // ★ 結果にも休憩時間を追加 ★
      });
...
        休憩時間: currentPoint.restTime, // ★ テーブルにも休憩時間を追加 ★
      });
    ...
    console.log('handleSimulate 後の startTime:', startTime); // ★ ここで出力 ★
    ...
    {draftManualPoints.length > 1 && (
      <button type="button" onClick={() => removeManualPoint(point.id)} style={{ marginLeft: '5px' }}>削除</button>
        ...
        {simulationResult.length > 0 && (
          <>
        {console.log('TimeGraph に渡す startTime:', startTime)}
