...
function App() {
  ...
  useEffect(() => {
    ...
    if (isGPXLoading) {
      ...
      if (rootElem) rootElem.classList.add(className);
    } else {
      htmlElem.classList.remove(className);
      bodyElem.classList.remove(className);
      if (rootElem) rootElem.classList.remove(className);
    }
  }, [isGPXLoading]);
 ...
 // GPX読み込みハンドラ
  ...
  }, [calculateDistance, setPointsData, loadManualPoints, setGpxFileName, setGpxFileLoaded, finishLoading]);
...
//  }, [setGpxFileLoaded, setPointsData, setSimulationResult, setTableData, setFirstKmArrivalTime]);
  ...
  const calculatePaceWithITRA = useCallback((basePaceInSecondsPerKm, itraIndexValue, distanceRatio) => {
    ...
    if (index > 825) {
      ...
      maxPenalty = 0.30;  // 最大30%失速
    } else {
      maxPenalty = 0.40;  // 最大40%失速
    }

    // レース後半（70%以上進行）でのみ失速を適用
    const lateStart = 0.7;
