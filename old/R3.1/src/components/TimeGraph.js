...
const TimeGraph = ({ results, pointsData, appliedManualPoints, elapsedTime }) => {
                        ...
                        if (context.dataset.label === '標高') {
                            ...
                            if (manualPoint && closestPoint && results[closestPointIndex]) {
                                ...
                                return [
                                  ...
                                  `標高: ${new Intl.NumberFormat('ja-JP', { style: 'decimal', minimumFractionDigits: 1, maximumFractionDigits: 1 }).format(resultData?.elevation)} m` // ★ ここを追加 ★
                                ];
                            }
                            return label;
    ...
    return (
        <div>
            <h2>シミュレーション結果</h2>
            <div>
              {elapsedTime !== undefined && <p>総所要時間: {formatTime(elapsedTime)}</p>}
